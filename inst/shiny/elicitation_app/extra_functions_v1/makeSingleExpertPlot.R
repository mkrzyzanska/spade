...
function(fit, d = "best", pl = -Inf, pu = Inf,
         ...
         lwd = 1, xlab, ylab, percentages,startDate,endDate,nBins,ybreaks,chips){
  ...
  if(d == "mirrorlogt"){ # mirror log student t
    ...
    if(d == "NS"){
        x<-fit$NS$values
        fx<-fit$NS$pdf_normalized
        print(paste("natural x",x))
        print(paste("natural fx:",fx))
        dist.title<-"Natural Cubic Spline Fit"
    }

     if(d == "MP"){
        x<-fit$MP$values
        fx<-fit$MP$pdf_normalized
        print(x)
        print(fx)
        dist.title<-"Monotonic P-spline"
    }
    
#### Code for re-drawing rectangles
x_bins <- seq(startDate, endDate, by = (endDate-startDate)/nBins) # Vertical bins (x-axis
width <- abs(x_bins[2]-x_bins[1])

#y_bins <- seq(0, max(fx), by = max(fx)/ybreaks)  # Horizontal bins (y-axis)
y_bins<-   seq(0, (0.95/(sum(chips)*width))*sum(chips), by = 0.95/(sum(chips)*width))
rect_counts <- chips  # How many rectangles in each x-bin

# Create a dataframe for plotting
# Create an empty dataframe to store rectangles
rectangles <- data.frame(xmin = numeric(),
                         xmax = numeric(),
                         ymin = numeric(),
                         ymax = numeric())

# Loop through each vertical bin
for (i in seq_along(x_bins[-length(x_bins)])) {
  x_min <- x_bins[i]
  x_max <- x_bins[i + 1]
  
  # Get the number of rectangles for this bin
  count <- rect_counts[i]
  
  # Ensure we don't exceed available y_bins
  if (count > length(y_bins) - 1) {
    count <- length(y_bins) - 1
  }
  
  # Stack rectangles from the bottom
  for (j in seq_len(count)) {
    y_min <- y_bins[j]
    y_max <- y_bins[j + 1]
    
    # Append to the dataframe
    rectangles <- rbind(rectangles, data.frame(xmin = x_min, xmax = x_max, 
...
##########################################################################  
    ...
    geom_rect(data = rectangles,inherit.aes=FALSE, aes(xmin = xmin, xmax = xmax, 
                                     ymin = ymin, ymax = ymax), 
            fill = "lightblue", color = "grey", alpha = 0.8)+
	  geom_line(size = lwd) +
	  labs(title = dist.title, x = xlab, y = "" )+
    ...
    if (all(new_ticks < 0)) {
      ...
      #  nBins=nBins-1
       ...
       #  Annotate BCE (left side of x = 1)
              annotate("text", x = 1 - 0.2, y = (0.95/(sum(chips)*width))*(sum(chips)+1) / 2, label = "BCE", color = "red", size = 6, hjust = 1) +
  
              # Annotate CE (right side of x = 1)
              annotate("text", x = 1 + 0.2, y = (0.95/(sum(chips)*width))*(sum(chips)+1) / 2, label = "CE", color = "red", size = 6, hjust = 0) 
     }

######### Plotting, regardless i
	if(percentages){
	  p1 <- p1 + scale_x_continuous(labels = scales::percent,  
	                                breaks = seq(startDate, endDate, by = (endDate-startDate)/nBins))
        
	}else{

	  p1 <- p1 + 
        scale_x_continuous(breaks = xticks,labels=round(abs(new_ticks),digits=2))+
                scale_y_continuous(breaks = seq(0, (0.95/(sum(chips)*width))*(sum(chips)+1), by = 0.95/(sum(chips)*width)))+
        xlab(x_lab)
       ...
       # scale_x_continuous(breaks = seq(startDate, endDate, by = (endDate-startDate)/nBins))+
                # scale_y_continuous(breaks = seq(0, max(fx), by = max(fx)/ybreaks))
        
       # geom_rect(data = rectangles,inherit.aes=FALSE, aes(xmin = xmin, xmax = xmax, 
        #                             ymin = ymin, ymax = ymax), 
        #    fill = "lightblue", color = "grey", alpha = 0.5)
                 
	}#}
    
   # p1<-p1+geom_rect(data = rectangles, aes(xmin = rectangles$xmin, xmax = rectangles$xmax, 
